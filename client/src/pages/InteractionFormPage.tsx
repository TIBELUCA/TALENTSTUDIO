...
import type { Interaction, Customer, Contact, Offer, JobOrder } from "@shared/schema";
...
import { ArrowLeft, Loader2, Save, Check, ChevronsUpDown, FileText, ClipboardList } from "lucide-react";
...
export default function InteractionFormPage() {
  ...
  const [, editParams] = useRoute("/crm/interactions/:id/edit");
  ...
  const { data: allOffers = [] } = useQuery<Offer[]>({ queryKey: ["/api/offers"] });
  const { data: allOrders = [] } = useQuery<JobOrder[]>({ queryKey: ["/api/orders"] });

  const { data: existingInteraction, isLoading: loadingInteraction } = useQuery<Interaction>({
    queryKey: ["/api/interactions", editId],
    queryFn: () => fetch(`/api/interactions/${editId}`, { credentials: "include" }).then(r => r.json()),
    enabled: !!editId,
  });

  const searchString = useSearch();
  const qp = useMemo(() => new URLSearchParams(searchString), [searchString]);
  const validInt = (v: string | null) => v && /^\d+$/.test(v) ? v : "";
  const qpCustomerId = validInt(qp.get("customerId"));
  const qpContactId = validInt(qp.get("contactId"));
  const qpJobOrderId = validInt(qp.get("jobOrderId"));
  const qpType = qp.get("type");
  const validTypes = ["email", "phone_call", "visit", "whatsapp", "video_call", "todo"];
  const resolvedType = qpType && validTypes.includes(qpType) ? qpType : undefined;

  const [form, setForm] = useState<InteractionFormData>({
    ...emptyForm,
    ...(qpCustomerId ? { customerId: qpCustomerId } : {}),
    ...(qpContactId ? { contactId: qpContactId } : {}),
    ...(qpJobOrderId ? { linkedJobOrderId: qpJobOrderId } : {}),
  ...
  useEffect(() => {
    if (!isEditing && !initialized && qpContactId && allContacts.length > 0) {
      const contact = allContacts.find(c => String(c.id) === qpContactId);
      if (contact?.customerId) {
        if (!qpCustomerId) {
          setForm(f => ({ ...f, customerId: String(contact.customerId) }));
        } else if (qpCustomerId !== String(contact.customerId)) {
          setForm(f => ({ ...f, contactId: "" }));
        }
      }
      setInitialized(true);
    }
  }, [isEditing, initialized, qpContactId, qpCustomerId, allContacts]);
  ...
  useEffect(() => {
    if (isEditing && existingInteraction && !initialized) {
      setForm({
        ...
        linkedJobOrderId: existingInteraction.linkedJobOrderId ? String(existingInteraction.linkedJobOrderId) : "",
      });
      setInitialized(true);
    }
  ...
  const companyOrders = useMemo(() => {
    if (!form.customerId) return [];
    return allOrders.filter(o => o.customerId === Number(form.customerId));
  }, [allOrders, form.customerId]);

  const filteredOrders = useMemo(() => {
    const q = orderSearch.toLowerCase().trim();
    return q
      ? companyOrders.filter(o => o.jobNumber.toLowerCase().includes(q))
      : companyOrders;
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      if (isEditing) {
        return apiRequest("PUT", `/api/interactions/${editId}`, data);
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    saveMutation.mutate({
      ...
      linkedJobOrderId: form.linkedJobOrderId ? Number(form.linkedJobOrderId) : null,
    });
  };
                        ...
                        <CommandItem
                          ...
                          onSelect={() => {
                            setForm(f => {
                              ...
                              const keepOrder = f.linkedJobOrderId && allOrders.some(o => String(o.id) === f.linkedJobOrderId && o.customerId === newCid);
                              return { ...f, customerId: String(c.id), contactId: "", linkedOfferId: keepOffer ? f.linkedOfferId : "", linkedJobOrderId: keepOrder ? f.linkedJobOrderId : "" };
                            });
            ...
            <Popover open={orderOpen} onOpenChange={setOrderOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  className="w-full justify-between font-normal"
                  disabled={!form.customerId || companyOrders.length === 0}
                  data-testid="select-order"
                >
                  {form.linkedJobOrderId && form.linkedJobOrderId !== "none"
                    ? (() => {
                        const o = allOrders.find(o => String(o.id) === form.linkedJobOrderId);
                        return o ? o.jobNumber : "Seleziona ordine (opzionale)";
                      })()
                    : !form.customerId ? "Seleziona prima una company" : companyOrders.length === 0 ? "Nessun ordine per questa company" : "Seleziona ordine (opzionale)"}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                <Command shouldFilter={false}>
                  <CommandInput
                    ...
                    data-testid="select-order-search"
                  />
                  <CommandList>
                    ...
                    <CommandGroup>
                      <CommandItem
                        value="none"
                        onSelect={() => {
                          setForm(f => ({ ...f, linkedJobOrderId: "" }));
                          setOrderOpen(false);
                          setOrderSearch("");
                        }}
                        data-testid="select-order-option-none"
                      >
                        <Check className={cn("mr-2 h-4 w-4", !form.linkedJobOrderId || form.linkedJobOrderId === "none" ? "opacity-100" : "opacity-0")} />
                        Nessun ordine
                      </CommandItem>
                      {filteredOrders.map(o => (
                        <CommandItem
                          ...
                          value={String(o.id)}
                          onSelect={() => {
                            setForm(f => ({ ...f, linkedJobOrderId: String(o.id) }));
                            setOrderOpen(false);
                            setOrderSearch("");
                          }}
                          data-testid={`select-order-option-${o.id}`}
                        >
                          <Check className={cn("mr-2 h-4 w-4", form.linkedJobOrderId === String(o.id) ? "opacity-100" : "opacity-0")} />
                          <ClipboardList className="mr-2 h-4 w-4 text-muted-foreground" />
                          <span className="truncate">{o.jobNumber}</span>
                        </CommandItem>
                      ))}
                    </CommandGroup>
                  </CommandList>
