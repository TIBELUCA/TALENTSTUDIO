...
- **Backend**:
  ...
  - Activity logging: every state change writes an `activity_logs` row with the new event types (`preventivo_inviato`, `preventivo_accettato`, `preventivo_rifiutato`, `campagna_creata`, `deliverable_pubblicato`, `pagamento_in_registrato`, `pagamento_out_registrato`).
